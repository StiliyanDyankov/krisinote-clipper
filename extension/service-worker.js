...
chrome.runtime.onMessage.addListener(async ({message}) => {
    ...
    if(message === 'LIFECYCLE_STATUS') {
        chrome.tabs.sendMessage(currentTabId, { type: 'LIFECYCLE_STATUS', id: tab.id });
    }

    if (message === 'Open new tab') {
        chrome.tabs.create({url: 'http://localhost:3000/login?loginFromExt=true'}, (tab) => {
            createtTabId = tab.id;
            chrome.scripting.executeScript({ target: { tabId: tab.id }, func: injectedFunction }).then((event)=> {
                // here to execute some logic - access to token
                chrome.tabs.sendMessage(currentTabId, { type: "USER_AUTHENTICATED", payload: event });
            })
        });
    }
...
async function injectedFunction() {
    const promise = new Promise((resolve) => {
        window.postMessage({message: "hey, its me, the injected script", type: "FROM_CONTENT_SCRIPT"})
